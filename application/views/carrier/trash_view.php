...
<div>
    <div id="dashboard-main">
        ...
        <div><h2>Trashed Carriers</h2></div>
            ...
            <table id="list_load" class="table table-hover table-bordered table-striped">
                ...
                <tbody>
                    ...
                    foreach ($carriers as $carriers => $row) {
                        ...
                        echo '<tr id="carrier_' . $row['idts_carrier'] . '">';
                        echo '<td>' . $i++ . '</td>';
//                        echo '<td><a href="#" id="' . $row['idts_carrier'] . '" data-toggle="popover" class="po" data-load_id="' . $row['idts_carrier'] . '">' . $row['name'] . '</a></td>';
                        ...
                        echo in_array('carrier/trash', $roles) ? '&nbsp; <a id="' . $row['idts_carrier'] . '" class="enable">Enable</a>' : '';
                        echo in_array('carrier/edit', $roles) || in_array('carrier/trash', $roles) ? '</td>' : '';
                        echo '</tr>';
                    }
                    ?>
...
//                "content":'<ul><li><a data-id="4" title="Edit this Load" href="load/update/'+popover+'"><i class="icon-pencil"></i> Edit</a> </li></ul>'
        ...
        $('body').on('click', '.enable', function (evt) {
            evt.preventDefault();
            var carrier = $(this);
            var id = carrier.attr('id');
            var r = confirm("Confirm enabling carrier?");

            if (r == true) {
                $.ajax({
                    type: "POST",
                    url: 'change_status/' + id + '/' + 1,
                    async: true,
                    dataType: "json",
                    beforeSend: function () {
                    ...
                    success: function (data) {
                        if (data.status == 1) {
                            $('#carrier_' + id).remove();
                            console.log('carrier enabled');
                        } else {
                            alert('Carrier could not be enabled. Please contact administrator.');
                        }
                    }
                });
            }
